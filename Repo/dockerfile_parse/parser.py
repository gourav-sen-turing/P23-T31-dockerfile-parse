...
class DockerfileParser(object):
            ...
            if '=' not in words[0]:  # LABEL/ENV name value
                ...
                if WordSplitter(words[0]).dequote() == instr_key:
                    if instr_value is None:
                        ...
                        # Keep the key in words[0], replace all remaining words with the new value
                        words = [words[0], quote(instr_value)]
                ...
                for index, token in enumerate(words):
                    ...
                    if WordSplitter(key).dequote() == instr_key:
                        if instr_value is None:
                            ...
                            # Preserve the key's original quoting if possible
                            words[index] = "{0}={1}".format(key, quote(instr_value))
    ...
    def _add_instruction(self, instruction, value):
        ...
        if (instruction == 'LABEL' or instruction == 'ENV') and len(value) == 2:
            # value is a tuple of (key, val)
            new_line = instruction + ' ' + '='.join(map(quote, value)) + '\n'
        else:
            new_line = '{0} {1}\n'.format(instruction, value)
        if new_line:
            lines = self.lines
            # Find a good position to insert (after last instruction of same type, or at end)
            insert_pos = len(lines)
            for i, insn in enumerate(self.structure):
                if insn['instruction'] == instruction:
                    insert_pos = insn['endline'] + 1
